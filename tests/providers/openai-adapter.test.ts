...
import { createOpenAIAdapter, OpenAIAdapter } from "../../src/providers/adapters/openai-adapter.js";
...
describe("OpenAI Adapter", () => {
  ...
  describe("createClientWrapper", () => {
    const mockClient: MockOpenAIClient = {
      chat: {
        completions: {
          create: async (_options) => ({ id: "test", choices: [{ message: { content: "response" } }] })
        }
      },
      completions: {
        create: async (_options) => ({ id: "test", choices: [{ text: "response" }] })
      },
      embeddings: {
      ...
      beta: {
        threads: {
          messages: {
            create: async (_threadId, _message) => ({ id: "msg_123", content: [] })
          },
          runs: {
            create: async (_threadId, _options) => ({ id: "run_123", status: "queued" })
          }
        }
      }
    };
  ...
  describe("configuration updates", () => {
    it("should allow config updates", () => {
      const adapter = createOpenAIAdapter({ policies: ["EMAIL_ADDRESS"] as PolicyName[] });
  ...
  describe("error handling", () => {
    it("should handle missing OpenAI client gracefully", async () => {
      ...
      await expect(wrapper.chat.completions.create({
        model: "gpt-4",
        messages: []

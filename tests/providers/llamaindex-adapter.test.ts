...
import { createLlamaIndexAdapter, LlamaIndexAdapter } from "../../src/providers/adapters/llamaindex-adapter.js";
...
describe("LlamaIndex Adapter", () => {
  ...
  describe("transform method", () => {
    it("should transform string input", async () => {
      ...
      const result = await adapter.transform("Contact john@example.com or call 555-123-4567");
    ...
    it("should throw error for invalid input types", async () => {
      const adapter = createLlamaIndexAdapter();
      await expect(adapter.transform(123 as unknown as string)).rejects.toThrow("Invalid input type");
    });

    it("should throw error for missing nodes in array", async () => {
      const adapter = createLlamaIndexAdapter();
      const nodesWithNull = [new MockNode("test"), null] as unknown as MockNode[];
      
      await expect(adapter.transform(nodesWithNull)).rejects.toThrow("Missing node at index 1");
    });
    ...
    it("should preserve node metadata", async () => {
      ...
      const nodes = [
        new MockNode("Hello world", { author: "john@example.com", other: "value" })
      ];
  ...
  describe("configuration updates", () => {
    it("should allow config updates", () => {
      const adapter = createLlamaIndexAdapter({ policies: ["EMAIL_ADDRESS"] as PolicyName[] });
